...
async function main() {
    ...
    if ($.activityInfo.pre.buttonId === 1) {
        await takePostRequest('getPoints');
        await $.wait(2000);
    }
    ...
    switch (type) {
        ...
        case 'getPoints':
            if (data && data.code === '0' && data.result) {
                console.log(`进行上一期瓜分：${data.result.title}`);
            } else {
                console.log(JSON.stringify(data));
            }
            break;
...
async function takeGetRequest(type) {
    ...
    switch (type) {
        case 'list':
            body = `{"modelId":"${$.modelId}","plateCode":3}`;
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'beansLottery':
            ...
            break;
        case 'getPoints':
            bodyInfo = `{"activityId":"${$.activityInfo.pre.activityId}"}`;
