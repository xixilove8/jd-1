...
芥么小程序签到领现金
活动地址：微信小程序 - 芥么
活动时间：2021-09-16 - 2022-12-31
20 1,12,21 * * * jd_jm_sign.js, tag=芥么小程序签到领现金, enabled=true
 */
const $ = new Env('芥么小程序签到领现金');
...
const linkId = "KRFM89OcZwyjnyOIPyAZxA";
...
async function main() {
  try {
    await sign();//芥么小程序签到
    await signPrizeDetailList();//芥么小程序提现
    await showMsg()
  } catch (e) {
    $.logErr(e)
  }
...
async function sign() {
  return new Promise(resolve => {
    const body = {"linkId": linkId,"serviceName":"dayDaySignGetRedEnvelopeSignService","business":1};
    const options = {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              if (data.data.retCode === 0) {
                ...
                console.log(`${$.name}：签到失败:${data.data.retMessage}\n`);
              }
            } else {
              console.log(`${$.name}：签到异常:${JSON.stringify(data)}\n`);
            }
          }
        }
      } catch (e) {
...
function signPrizeDetailList() {
  return new Promise(resolve => {
    const body = {"linkId":linkId,"serviceName":"dayDaySignGetRedEnvelopeSignService","business":1,"pageSize":20,"page":1};
    const options = {
      url: `https://api.m.jd.com`,
            ...
            if (data.code === 0) {
              if (data.data.code === 0) {
                ...
                for (let code of list) {
                  console.log(`${$.name}，去提现${code['prizeValue']}现金\n`);
                  message += `提现${code['prizeValue']}微信现金`
                  await apCashWithDraw(code['id'], code['poolBaseId'], code['prizeGroupId'], code['prizeBaseId']);
                }
              } else {
                console.log(`${$.name}查询奖品：失败:${JSON.stringify(data)}\n`);
              }
            } else {
              console.log(`${$.name}查询奖品：异常:${JSON.stringify(data)}\n`);
            }
          }
        }
      } catch (e) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              if (data.data.status === "310") {
                console.log(`${$.name} 提现现金成功！`)
                message += `提现至微信钱包成功！`;
              } else {
                console.log(`${$.name} 提现现金：失败:${JSON.stringify(data)}\n`);
              }
            } else {
              console.log(`${$.name} 提现现金：异常:${JSON.stringify(data)}\n`);
            }
          }
        }
      } catch (e) {
