/*
新版京喜财富岛 热气球接待游客、雇佣导游、收集4个建筑京币
目前发现一天上限是501次，故一天执行24次，一次招待21个游客即可到达上限。
雇佣导游一个小时可雇佣一次
20 * * * * jd_jxbfd_balloon.js
更新日期：2021-07-22
 */
const $ = new Env("京喜财富岛-热气球接待游客");
          ...
          if (err) {
            ...
            if (data) {
              if (data['iRet'] === 0) {
                ...
                console.log(`账号 ${$.index} ${$.UserName} 接待游客失败: ${data['sErrMsg']}, iRet: ${data['iRet']}\n`)
              }
            }
          }
        } catch (e) {
...
function EmployTourGuide(body, timeout = 2000) {
  return new Promise(async (resolve) => {
    setTimeout(() => {
