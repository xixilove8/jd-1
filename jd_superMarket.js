...
async function jdSuperMarket() {
  try {
    ...
    // await smtgHome();
    await receiveUserUpgradeBlue();//领取店铺升级奖励（蓝币）
    await Home();
    await helpAuthor();
  } catch (e) {
    $.logErr(e)
        ...
        try {
          if (err) {
            ...
            if  ($.data.data.bizCode === 0) {
              $.currentShopId = $.data.data.result.currentShopId;
              if ($.currentShopId) {
                if (shopType === 1) {
                  console.log('限时超市-开始收取营业额')
                  const receiveCoinNewBody = {"type": 7, shopId: $.currentShopId,"channel":1};
                  await smtg_receiveCoinNew(receiveCoinNewBody);
                  console.log('限时超市-开始收取小费')
                  await receiveBlueCoinNew(0, $.currentShopId);
                } else if (shopType === 0) {
                  console.log('经典超市-开始收取营业额')
                  const receiveCoinNewBody = {"type": 4, shopId: $.currentShopId,"channel":1};
                  await smtg_receiveCoinNew(receiveCoinNewBody);
                  console.log('经典超市-开始收取小费')
                  await receiveBlueCoinNew(0, $.currentShopId, 2);
                }
              }
            } else {
...
//新，收取营业额
function smtg_receiveCoinNew(body) {
  return new Promise((resolve) => {
    const { type } = body;
    const op = {
      url: `${JD_API_HOST}?functionId=smtg_receiveCoin&appid=jdsupermarket&clientVersion=8.0.0&client=m&eid=&body=${escape(JSON.stringify(body))}&t=${Date.now()}`,
      headers: {
        'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
        'Host': 'api.m.jd.com',
        'Cookie': cookie,
    ...
    $.get(op, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if  ($.data.data.bizCode === 0) {
            if (type === 7) {
              console.log(`营业额 收取成功，获得超市币：${data.data.result.receivedTurnover}，当前总超市币：${data.data.result.currentTurnover}`)
            } else if (type === 4) {
              console.log(`营业额 收取成功，获得营业额：${data.data.result.receivedTurnover}，当前总营业额：${data.data.result.currentTurnover}/${data.data.result.maxTurnover}`)
            } else if (type === 5) {
              console.log(`店铺升级奖励 收取成功，获得蓝币：${data.data.result.receivedBlue}，当前总蓝币：${data.data.result.totalBlue}`);
            }
          } else {
            console.log(`${$.toStr(data)}\n`)
          }
...
//新 - 收取小费
function receiveBlueCoinNew(timeout = 0, shopId = '', type = 6) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      const body = {"type": type,shopId,"channel":"18"}
      const op = {
        url: `${JD_API_HOST}?functionId=smtg_receiveCoin&appid=jdsupermarket&clientVersion=8.0.0&client=m&eid=&body=${escape(JSON.stringify(body))}&t=${Date.now()}`,
        headers: {
          'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
      ...
      $.get(op, async (err, resp, data) => {
        try {
          if (err) {
            ...
            await receiveBlueCoinNew(5000, shopId, type);
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
...
async function receiveUserUpgradeBlue() {
  ...
  if ($.userUpgradeBlueVos && $.userUpgradeBlueVos.length > 0) {
    for (let item of $.userUpgradeBlueVos) {
      const receiveCoinNewBody = {"type": 5, shopId: $.currentShopId,"channel":18, "id": item.id};
      await smtg_receiveCoinNew(receiveCoinNewBody);
      await $.wait(500)
    }
  } else {
    console.log(`当前暂无店铺升级奖励\n`)
  }
}
async function Home() {
      ...
      try {
        if (err) {
          ...
          if (data && data.data['bizCode'] === 0) {
            const { shopId, shelfList, merchandiseList, level, userUpgradeBlueVos } = data.data['result'];
...
async function barGain() {
  let res = await getAuthorShareCode2('https://cdn.jsdelivr.net/gh/gitupdate/updateTeam@master/shareCodes/jd_barGain.json'), res2 = [];
...
function _618(redEnvelopeId, inviter, helpType = '1', linkId = $.linkId) {
  return new Promise(resolve => {
    $.get({
