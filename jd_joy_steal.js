/*
Last Modified time: 2021-8-21 10:22:37
  ...
  try {
    ...
    if ($.getFriendsData && $.getFriendsData.success) {
      ...
      if ($.getFriendsData && $.getFriendsData.datas && $.getFriendsData.datas.length  > 0) {
        ...
        console.log(`暂无好友。${$.toStr($.getFriendsData)}\n`);
      }
    } else {
...
function getFriends(currentPage = '1') {
  return new Promise(resolve => {
    let opt = {
      url: `//draw.jdfcloud.com//common/pet/api/getFriends?itemsPerPage=20&currentPage=${currentPage * 1}&reqSource=weapp&invokeKey=${invokeKey}`,
      method: "GET",
      data: {},
      credentials: "include",
...
function getCoinChanges() {
  return new Promise(resolve => {
    let opt = {
      url: `//jdjoy.jd.com/common/pet/getCoinChanges?changeDate=${Date.now()}&reqSource=h5&invokeKey=${invokeKey}`,
...
function taskUrl(functionId, friendPin) {
  let opt = {
    url: `//jdjoy.jd.com/common/pet/${functionId}?friendPin=${encodeURI(friendPin)}&reqSource=h5&invokeKey=${invokeKey}`,
    // url: `//draw.jdfcloud.com/common/pet/getPetTaskConfig?reqSource=h5`,
    method: "GET",
    data: {},
    credentials: "include",
